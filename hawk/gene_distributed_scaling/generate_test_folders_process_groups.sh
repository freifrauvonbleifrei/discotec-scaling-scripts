...
for m in {2,3,4}; do #{1..6}
	i=9
	TWO_TO_I=$((2 ** i))
	TWO_TO_M=$((2 ** m))
	echo $TWO_TO_I times $TWO_TO_M
	FOLDER=n_gene_weak_${TWO_TO_I}x${TWO_TO_M}
...
#	# replace resolutions and parallelizations
	...
	for (( j=0; j<$(($i  - 5)); j++ )) do
		k=$((($j+3) % 4))
		ADD_ARRAY_P[$k]=$((ADD_ARRAY_P[$k] + 1))
		p[$k]=$((p[$k] * 2))
	done
	echo ADD_ARRAY_P ${ADD_ARRAY_P[@]}
